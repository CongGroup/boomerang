...
private:
    ...
    std::vector<std::unique_ptr<BoomerangService::Stub>> connections_enode_;
...
public:
    explicit BoomerangClient(size_t id, size_t user_num, size_t parallel_num, const std::string& network_filename, size_t enode_num, size_t bnode_num) {
        // Set id
        id_ = id;
            ...
            for (size_t idx = 0;idx < parallel_num;++idx) {
                threads.emplace_back([=, &mapping_enodeid_counter, &counter_mtx] {
                    ...
                    for (size_t k = std::floor(idx * chip);k < std::floor((idx + 1) * chip);++k) {
                        ...
                        send_pkt2enode(random_enode, users_[k], std::string(pkt_buffer, pkt_buffer + pkt_size), idx);
                    }
                    });
            }
            for (size_t idx = 0;idx < parallel_num;++idx) {
...
private:
    void add_connect2enode(const std::string& addr) {
        connections_enode_.emplace_back(BoomerangService::NewStub(grpc::CreateChannel(addr, grpc::InsecureChannelCredentials())));
...
int main(int argc, char** argv)
{
    std::map<std::string, docopt::value> docopt_args = docopt::docopt(
        ...
        "Boomerang Client");  // version string
    ...
    BoomerangClient boomerang_client(docopt_args["--id"].asLong(), docopt_args["--user-num"].asLong(), docopt_args["--parallel-num"].asLong(), docopt_args["--config-path"].asString(), docopt_args["--enode-num"].asLong(), docopt_args["--bnode-num"].asLong());

    boomerang_client.recv_pkts_from_enode_mt();
    boomerang_client.start_round(docopt_args["--round-num"].asLong(), PKT_SIZE);

    boomerang_client.join();  //blocks forever
    return 0;
}